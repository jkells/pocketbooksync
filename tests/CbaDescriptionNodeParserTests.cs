...
namespace PocketBookSync.tests
{
    public class CbaDescriptionNodeParserTests
    {
        ...
        [Fact]
        public void is_valid_false_for_unrecognised_rows()
        {
            var html =
                @"<td class=""arrow"">Blah</td>";
            var document = new HtmlDocument();
            document.LoadHtml(html);
        ...
        {
            var html =
                @"<td class=""arrow""><span class=""description""><span class=""merchant"">Direct Credit 123456 Company<br>MY SALARY</span></span><b aria-hidden=""true"" role=""presentation"" class=""barrow"" tabindex="" - 1""><bdo></bdo></b></td>";
            var document = new HtmlDocument();
            document.LoadHtml(html);
